...
const GroupScreen: React.FC<GroupScreenProps> = ({ route }) => {
  ...
  const navigation = useNavigation<StackNavigationProp<RootStackParamList, 'GroupDetail'>>();
  ...
  const calculateTotalOwed = () => {
    ...
    for (const exp of groupExpenses) {
      for (const split of exp.splits) {
        if (split.name === 'You' && split.amount) {
          total += split.amount;
        }
      }
    }
      ...
      <View style={styles.header}>
        ...
        <View style={styles.avatarGroup}>
          {group.members.map((m, idx) => (
            <View key={idx} style={[styles.avatar, { backgroundColor: m.color }]}>
              <Image source={m.avatar} style={styles.avatarImage} />
            </View>
          ))}
        </View>
      </View>

      <View style={styles.settleDebtsSection}>
        <TouchableOpacity style={styles.settleButton} onPress={handleSettleDebts}>
          <Text style={styles.settleButtonText}>Settle Debts</Text>
          <Text style={styles.badge}>${calculateTotalOwed()}</Text>
        </TouchableOpacity>
      </View>
        ...
        {groupExpenses.map(exp => (
          <View key={exp.id} style={styles.expenseCard}>
            <View style={styles.expenseHeader}>
              <Text style={styles.expenseTitle}>{exp.name}</Text>
            ...
            {exp.notes ? <Text style={styles.notes}>Notes: {exp.notes}</Text> : null}
            {exp.splits.map((split, index) => (
              <View key={index} style={styles.splitRow}>
                <Text>{split.name} owes</Text>
