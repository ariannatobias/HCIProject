...
const GroupMemberScreen: React.FC<GroupMemberScreenProps> = ({ route, navigation }) => {
  ...
  const { addGroup, currentUser } = useGroups();
  const { user } = useUser();
  ...
  useEffect(() => {
    const loadUsers = async () => {
      try {
        ...
        // Include current user by default in group members
        if (user) {
          ...
          setGroupMembers([currentUserAsMember]);
        }
      } catch (err) {
  ...
  const handleRemoveMember = (member: Member) => {
    // Prevent removing the current user from the group
    if (member.id === user?.id.toString()) return;
